...
.stApp { background: #0a0e27; color: #f8fafc; }
...
animation: backgroundScroll 60s linear infinite; pointer-events:none; z-index:0;}
@keyframes backgroundScroll {0%{background-position:0 0,40px 60px,130px 270px,70px 100px;}
100%{background-position:200px 200px,240px 260px,330px 470px,270px 300px;}}
section[data-testid="stSidebar"] { background: rgba(15, 23, 42, 0.75) !important; backdrop-filter: blur(20px) saturate(180%); border-right: 1px solid rgba(148, 163, 184, 0.1);}
div[data-testid="metric-container"] { background: rgba(30,41,59,0.4); backdrop-filter: blur(16px); padding:20px; border-radius:20px; border:1px solid rgba(148,163,184,0.1); transition: transform 0.3s ease;}
div[data-testid="metric-container"]:hover { transform: translateY(-5px); border-color: rgba(99,102,241,0.5); box-shadow:0 10px 30px -10px rgba(99,102,241,0.3);}
div[data-testid="stMetricValue"] { font-size:32px !important; background: linear-gradient(135deg,#60a5fa 0%,#a78bfa 100%); -webkit-background-clip:text; -webkit-text-fill-color:transparent; font-weight:700 !important;}
.stButton>button { border-radius:12px; height:3.5em; background:linear-gradient(135deg,#3b82f6 0%,#8b5cf6 50%,#ec4899 100%); background-size:200% 200%; color:white; border:none; font-weight:700; width:100%; transition: all 0.3s ease;}
.stButton>button:hover { background-position:100% 0; transform:scale(1.02); box-shadow:0 8px 25px rgba(59,130,246,0.4);}
h1 { background: linear-gradient(135deg,#60a5fa 0%,#a78bfa 50%,#ec4899 100%); -webkit-background-clip:text; -webkit-text-fill-color:transparent; font-weight:800 !important; }
h2,h3 { color:#e2e8f0 !important; }
.prediction-card { background: rgba(30,41,59,0.6); backdrop-filter: blur(20px); padding:40px; border-radius:24px; border:1px solid rgba(255,255,255,0.1); text-align:center; animation: glow 3s infinite alternate;}
@keyframes glow { from { box-shadow:0 0 20px rgba(59,130,246,0.1);} to { box-shadow:0 0 40px rgba(59,130,246,0.3);}}
::-webkit-scrollbar { width:10px;} ::-webkit-scrollbar-track { background:#0f172a;} ::-webkit-scrollbar-thumb { background:#3b82f6; border-radius:5px;}
</style>
""", unsafe_allow_html=True)

# --- GLOBAL MODEL ACCURACY PLACEHOLDERS ---
rf_acc, xgb_acc = 0, 0
...
def train_models(X, y):
    ...
    xgb_model = XGBClassifier(n_estimators=150, max_depth=6, learning_rate=0.1, random_state=42, use_label_encoder=False, eval_metric='logloss')
...
# --- DEEP INSIGHTS ---
    ...
    with col_a:
        ...
        impact_data.columns = ['AI Impact Perception', 'count']
...
# --- PREDICTION LAB ---
    ...
    for col in features_emp:
        ml_df_emp[col] = enc_emp[col].transform(ml_df_emp[col].astype(str))
    ...
    for col in features_trust:
        ml_df_trust[col] = enc_trust[col].transform(ml_df_trust[col].astype(str))
    ml_df_trust['Trust in AI'] = enc_trust['Trust in AI'].transform(ml_df_trust['Trust in AI'].astype(str))

    rf_e, xgb_e, rf_acc_e, xgb_acc_e, _, _ = train_models(ml_df_emp[features_emp], ml_df_emp['Employment Status'])
    rf_t, xgb_t, rf_acc_t, xgb_acc_t, _, _ = train_models(ml_df_trust[features_trust], ml_df_trust['Trust in AI'])
...
# --- MODEL ARENA ---
    ...
    for c in ml_df.columns:
        ml_df[c] = le_dict[c].transform(ml_df[c].astype(str))
